...
class UserController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Handle colleagues only filter
        if ($request->filled('colleagues_only') && $request->get('colleagues_only') === '1') {
            $currentUser = Auth::user();
            if ($currentUser && $currentUser->department_id) {
                $query->where('department_id', $currentUser->department_id);
            } else {
                // If current user has no department, show only users with no department
    ...
    public function create(Request $request)
    {
        ...
        // Get pre-selected department and manager from URL parameters
        $preselectedDepartmentId = $request->get('department_id');
        $preselectedManagerId = $request->get('manager_id');
        
        return view('users.create', compact('departments', 'users', 'roles', 'preselectedDepartmentId', 'preselectedManagerId'));
    }
        ...
        $request->validate([
            ...
            'work_phones.*.id' => [
                ...
                function ($attribute, $value, $fail) {
                    // Skip validation for temporary IDs (e.g., "temp_0")
                    if ($value && !str_starts_with($value, 'temp_')) {
                        if (!is_numeric($value) || !\App\Models\UserPhone::where('id', $value)->exists()) {
                            $fail('The selected phone ID is invalid.');
                        }
                    }
                },
            ],
