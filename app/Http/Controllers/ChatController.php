...
public function startChatWithUser(Request $request)
    ...
    public function sendStaticMessage(Request $request)
    {
        try {
            ...
            // Update unread count for other participants (if column exists)
            try {
                ...
                \Log::debug('unread_count column does not exist, skipping update');
            }
            ...
            try {
                ...
                // This allows messages to be saved even if Reverb is not running
                \Log::warning('Failed to broadcast message: ' . $e->getMessage());
                // In development, you might want to see this error
                if (config('app.debug')) {
                    \Log::debug('Broadcast error details: ' . $e->getTraceAsString());
                }
            }
            ...
            return response()->json([
                ...
                'message' => [
                    ...
                    'sender_id' => $message->user_id, // للتوافق مع JavaScript
                    'content' => $message->message,
                    'message' => $message->message, // للتوافق مع الكود القديم
                    'type' => $message->type,
                    'user_name' => $user->name,
            ...
            \Log::error('Error sending static message: ' . $e->getMessage(), [
            ...
            $messages = $chatRoom->messages()
                ...
                ->map(function($message) use ($user) {
                    return [
                        ...
                        'content' => $message->message, // محتوى الرسالة
                        'type' => $message->type,
                        'created_at' => $message->created_at->toISOString(),
                        'updated_at' => $message->updated_at->toISOString(),
                        'is_own' => $message->user_id === $user->id, // تحديد إذا كانت الرسالة من المستخدم الحالي
                        'user' => [
                            'id' => $message->user->id,
