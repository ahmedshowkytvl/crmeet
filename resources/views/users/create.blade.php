            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3" id="department-field">
                    <label for="department_id" class="form-label">
                        {{ __('messages.department') }} <span class="text-danger">*</span>
                        @if(isset($preselectedDepartmentId))
                            <span class="badge bg-success ms-1" title="تم اختياره تلقائياً من القسم">
                                <i class="fas fa-magic"></i> مُختار تلقائياً
                            </span>
                        @endif
                    </label>
                    <select class="form-select @error('department_id') is-invalid @enderror {{ isset($preselectedDepartmentId) ? 'border-success' : '' }}" id="department_id" name="department_id">
                        <option value="">{{ __('messages.select_department') }}</option>
                        @foreach($departments as $department)
                            <option value="{{ $department->id }}" {{ (old('department_id', $preselectedDepartmentId ?? '') == $department->id) ? 'selected' : '' }}>
                                {{ $department->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3" id="manager-field">
                    <label for="manager_id" class="form-label">
                        {{ __('messages.manager') }}
                        @if(isset($preselectedManagerId))
                            <span class="badge bg-info ms-1" title="تم اختياره تلقائياً كمدير القسم">
                                <i class="fas fa-user-tie"></i> مدير القسم
                            </span>
                        @endif
                    </label>
                    <select class="form-select @error('manager_id') is-invalid @enderror {{ isset($preselectedManagerId) ? 'border-info' : '' }}" id="manager_id" name="manager_id">
                        <option value="">{{ __('messages.select_manager') }}</option>
                        @foreach($users as $manager)
                            <option value="{{ $manager->id }}" {{ (old('manager_id', $preselectedManagerId ?? '') == $manager->id) ? 'selected' : '' }}>
                                {{ $manager->name }}
                            </option>
                        @endforeach
                    </select>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Show welcome message if fields are pre-selected
    @if(isset($preselectedDepartmentId) || isset($preselectedManagerId))
        showWelcomeMessage();
    @endif
    ...
    function showWelcomeMessage() {
        ...
        alertDiv.innerHTML = `
            ...
            <strong>تم التعرف على القسم تلقائياً!</strong> 
            تم اختيار القسم والمدير المناسبين بناءً على القسم الذي تم النقر منه.
