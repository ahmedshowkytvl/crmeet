...
document.addEventListener('DOMContentLoaded', function() {
            ...
            console.log('DOM fully loaded, looking for elements...');
            ...
            function openNewChatModal() {
                console.log('openNewChatModal called');
                console.log('newChatModal element:', newChatModal);
                if (newChatModal) {
                    ...
                    console.error('newChatModal element not found!');
                }
            }
            ...
            console.log('All buttons with admin-btn class:', document.querySelectorAll('.admin-btn'));
            if (newChatBtn) {
                ...
                console.log('All admin buttons found:', allButtons);
                allButtons.forEach((btn, index) => {
                    console.log(`Button ${index}:`, btn.id, btn.innerHTML);
                });
            }
        ...
        // إيقاف الـ polling عند إخفاء الصفحة
    ...
    messageInput.addEventListener('keypress', function(e) {
        ...
        async function loadConversationsList(type = 'all', filter = 'all') {
            ...
            try {
                ...
                if (data.success) {
                    console.log('Conversations loaded successfully:', data.conversations.length, 'conversations');
        ...
        function renderConversations(conversations, filter = 'all') {
            ...
            conversationsList.innerHTML = filteredConversations.map(conversation => {
                console.log('Processing conversation:', conversation.id, conversation.display_name);
        ...
        function createMessageElement(message) {
            ...
            // استخدام is_own مباشرة من الخادم
            const isOwn = message.is_own === true;
            
            console.log('Message is_own field:', message.is_own);
            console.log('Is own message:', isOwn);
            
            messageElement.className = `message ${isOwn ? 'own' : ''} fade-in`;
            messageElement.dataset.messageId = message.id;
            
            console.log('Message element class:', messageElement.className);
            
            // استخدام البيانات الصحيحة من الخادم
            const avatarHtml = message.user_avatar ?
                `<img src="${message.user_avatar}" alt="${message.user_name}">` :
                `<div class="default-avatar">${message.user_name ? message.user_name.charAt(0) : 'U'}</div>`;
            
            const timeAgo = message.time || getTimeAgo(message.created_at);
            console.log('Time ago:', timeAgo, 'created_at:', message.created_at);
            
            messageElement.innerHTML = `
                <div class="message-avatar">
                    ${avatarHtml}
                    <div class="avatar-tooltip">${message.user_name || 'Unknown'}</div>
                </div>
                <div class="message-content">
        ...
        function getTimeAgo(dateString) {
            ...
            if (!dateString) {
                console.log('No date string provided, returning "الآن"');
                return 'الآن';
            }
            ...
            console.log('Is valid date:', !isNaN(messageDate.getTime()));
            
            if (isNaN(messageDate.getTime())) {
                console.log('Invalid date, returning "الآن"');
                return 'الآن';
            }
            ...
            console.log('Diff ms:', diffMs, 'mins:', diffMins, 'hours:', diffHours, 'days:', diffDays);
        ...
        function sendMessage() {
            ...
            if (!messageInput || !messageInput.value.trim()) {
                console.log('No message input or empty message');
                return;
            }
            ...
            // إضافة رسالة مؤقتة فوراً للواجهة
            const tempMessageId = 'temp_' + Date.now();
            console.log('Creating temp message with ID:', tempMessageId);
            const tempMessageElement = createTempMessageElement(message, tempMessageId);
            console.log('Temp message element created:', tempMessageElement);
            messagesContainer.appendChild(tempMessageElement);
            
            // Scroll to bottom
            messagesContainer.scrollTop = messagesContainer.scrollHeight;
            console.log('Temp message added to container');
            ...
            .then(data => {
                ...
                console.log('Temp message ID:', tempMessageId);
                console.log('Temp element found:', tempElement);
                
                if (tempElement) {
                    console.log('Temp element HTML before removal:', tempElement.outerHTML);
                    console.log('Temp element classes:', tempElement.className);
                    tempElement.remove();
                    console.log('Temp message removed successfully');
                } else {
                    console.log('Temp message not found:', tempMessageId);
                    console.log('All elements with temp ID:', document.querySelectorAll(`[id*="temp_"]`));
                }
                
                if (data.success) {
                    console.log('=== Adding Real Message ===');
                    console.log('Message sent successfully:', data.message);
                    console.log('Message data structure:', JSON.stringify(data.message, null, 2));
                    
                    // إضافة الرسالة الحقيقية
                    const realMessageElement = createMessageElement(data.message);
                    console.log('Created real message element:', realMessageElement);
                    console.log('Real message element HTML:', realMessageElement.outerHTML);
                    console.log('Real message element classes:', realMessageElement.className);
                    
                    messagesContainer.appendChild(realMessageElement);
                    console.log('Real message added to container');
                    console.log('Container children count:', messagesContainer.children.length);
                    ...
                    showErrorMessage('فشل في إرسال الرسالة');
                }
            })
            .catch(error => {
        ...
        // دالة إنشاء رسالة مؤقتة
        function createTempMessageElement(message, tempId) {
            console.log('=== createTempMessageElement called ===');
            console.log('Message:', message, 'Temp ID:', tempId);
            
            const messageElement = document.createElement('div');
            messageElement.id = tempId;
            messageElement.className = 'message own temp-message fade-in';
            
            console.log('Message element created with class:', messageElement.className);
            const currentUserAvatar = document.querySelector('.admin-avatar-img');
            const currentUserName = document.querySelector('.admin-name').textContent.trim();
            
            let avatarHtml;
            if (currentUserAvatar && currentUserAvatar.src) {
                avatarHtml = `<img src="${currentUserAvatar.src}" alt="${currentUserName}">`;
            } else {
                avatarHtml = `<div class="default-avatar">${currentUserName.charAt(0)}</div>`;
            }
            
            messageElement.innerHTML = `
                <div class="message-avatar">
                    ${avatarHtml}
                    <div class="avatar-tooltip">${currentUserName}</div>
                </div>
                <div class="message-content">
            ...
            console.log('Temp message HTML set:', messageElement.innerHTML);
            console.log('=== createTempMessageElement completed ===');
            return messageElement;
        }
