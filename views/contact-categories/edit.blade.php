...
@section('title', __('messages.edit_category') . ' - ' . __('messages.system_title'))

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2><i class="fas fa-edit me-2"></i>{{ __('messages.edit_category') }}</h2>
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('contact-categories.update', $contactCategory) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                <div class="col-md-6">
                    <h5 class="text-primary mb-3">
                        <i class="fas fa-language me-2"></i>{{ __('messages.arabic_info') }}
                    </h5>
                    
                    <div class="mb-3">
                        <label for="name" class="form-label">{{ __('messages.name_ar') }} <span class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('name') is-invalid @enderror" 
                               id="name" name="name" value="{{ old('name', $contactCategory->name) }}" required>
                        @error('name')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" 
                                  id="description" name="description" rows="3">{{ old('description', $contactCategory->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-6">
                    <h5 class="text-primary mb-3">
                        <i class="fas fa-language me-2"></i>{{ __('messages.english_info') }}
                    </h5>
                    
                    <div class="mb-3">
                        <label for="name_en" class="form-label">{{ __('messages.name_en') }} <span class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('name_en') is-invalid @enderror" 
                               id="name_en" name="name_en" value="{{ old('name_en', $contactCategory->name_en) }}" required>
                        @error('name_en')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description_en') is-invalid @enderror" 
                                  id="description_en" name="description_en" rows="3">{{ old('description_en', $contactCategory->description_en) }}</textarea>
            ...
            <div class="row">
                <div class="col-md-6">
                    <h5 class="text-primary mb-3">
                        <i class="fas fa-palette me-2"></i>{{ __('messages.appearance') }}
                    </h5>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <input type="color" class="form-control form-control-color @error('color') is-invalid @enderror" 
                                   id="color" name="color" value="{{ old('color', $contactCategory->color) }}" required>
                            <input type="text" class="form-control @error('color') is-invalid @enderror" 
                                   id="color_text" value="{{ old('color', $contactCategory->color) }}" readonly>
                        </div>
                        @error('color')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        <label for="icon" class="form-label">{{ __('messages.icon') }} <span class="text-danger">*</span></label>
                        <div class="input-group">
                            <span class="input-group-text" id="icon-preview">
                                <i class="{{ $contactCategory->icon }}" id="icon-display"></i>
                            </span>
                            <input type="text" class="form-control @error('icon') is-invalid @enderror" 
                                   id="icon" name="icon" value="{{ old('icon', $contactCategory->icon) }}" 
                            ...
                            <button type="button" class="btn btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#iconSelectorModal">
                                <i class="fas fa-search"></i> {{ __('messages.choose_icon') }}
                            </button>
                        </div>
                        <small class="form-text text-muted">{{ __('messages.icon_help') }}</small>
                        @error('icon')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('sort_order') is-invalid @enderror" 
                               id="sort_order" name="sort_order" value="{{ old('sort_order', $contactCategory->sort_order) }}" min="0">
                        @error('sort_order')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-check form-switch mb-3">
                        <input class="form-check-input" type="checkbox" id="is_active" name="is_active" 
                               value="1" {{ old('is_active', $contactCategory->is_active) ? 'checked' : '' }}>
            ...
            <div class="row mt-4">
                <div class="col-12">
                    <h5 class="text-primary mb-3">
                        <i class="fas fa-eye me-2"></i>{{ __('messages.preview') }}
                    </h5>
                    
                    <!-- معاينة باللغة الإنجليزية -->
                    <div class="card mb-3" id="category-preview-en">
                        <div class="card-header bg-light">
                            <h6 class="mb-0"><i class="fas fa-flag me-2"></i>{{ __('messages.english_preview') }}</h6>
                        </div>
                        <div class="card-body">
                            <div class="d-flex align-items-center">
                                <i class="{{ $contactCategory->icon }} me-2" id="preview-icon-en" style="color: {{ $contactCategory->color }}"></i>
                                <h5 class="mb-0" id="preview-name-en">{{ $contactCategory->name }}</h5>
                            </div>
                            <p class="text-muted mb-0 mt-2" id="preview-description-en">{{ $contactCategory->description ?: __('messages.category_description') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="card" id="category-preview-ar" dir="rtl">
                        <div class="card-header bg-light">
                            <h6 class="mb-0"><i class="fas fa-flag me-2"></i>{{ __('messages.arabic_preview') }}</h6>
                        </div>
                        <div class="card-body">
                            <div class="d-flex align-items-center">
                                <i class="{{ $contactCategory->icon }} me-2" id="preview-icon-ar" style="color: {{ $contactCategory->color }}"></i>
                                <h5 class="mb-0" id="preview-name-ar">{{ $contactCategory->name }}</h5>
                            </div>
                            <p class="text-muted mb-0 mt-2" id="preview-description-ar">{{ $contactCategory->description ?: __('messages.category_description') }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="d-flex justify-content-end mt-4">
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-save me-2"></i>{{ __('messages.update_category') }}
                </button>
            </div>
        </form>
    </div>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="iconSelectorModalLabel">
                    <i class="fas fa-icons me-2"></i>{{ __('messages.choose_icon') }}
                </h5>
            ...
            <div class="modal-body">
                ...
                <div class="mb-3">
                    <div class="input-group">
                        ...
                        <input type="text" class="form-control" id="iconSearch" placeholder="{{ __('messages.search_icons') }}">
                    </div>
                </div>
                
                <!-- Icon Categories -->
                <div class="mb-3">
                    <label class="form-label">{{ __('messages.icon_category') }}:</label>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" id="selectIconBtn" disabled>{{ __('messages.select') }}</button>
            </div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    nameInput.addEventListener('input', function() {
        previewName.textContent = this.value || '{{ __('messages."category_name") }}';
        // تحديث المعاينة الإنجليزية
        document.getElementById('preview-name-en').textContent = this.value || '{{ __('messages."category_name") }}';
        // تحديث المعاينة العربية
        document.getElementById('preview-name-ar').textContent = this.value || '{{ __('messages."category_name") }}';
    });

    // تحديث الوصف
    descriptionInput.addEventListener('input', function() {
        previewDescription.textContent = this.value || '{{ __('messages."category_description") }}';
        // تحديث المعاينة الإنجليزية
        document.getElementById('preview-description-en').textContent = this.value || '{{ __('messages."category_description") }}';
        // تحديث المعاينة العربية
        document.getElementById('preview-description-ar').textContent = this.value || '{{ __('messages."category_description") }}';
    });

    // تحديث لون الحدود
    previewCard.style.borderLeft = '4px solid {{ $contactCategory->color }}';
