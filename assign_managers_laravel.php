...
function assignManagersToEmployees() {
    ...
    echo "تم العثور على " . count($reportingRelations) . " علاقة إدارية\n\n";
    ...
    foreach ($reportingRelations as $employeeCode => $managerCode) {
        ...
        if ($employee && $manager) {
            ...
            if (!$employee) {
                echo "✗ لم يتم العثور على الموظف بكود: {$employeeCode}\n";
            }
            if (!$manager) {
                echo "✗ لم يتم العثور على المدير بكود: {$managerCode}\n";
            }
            $errorCount++;
        }
    }
    ...
    echo "\n=== ملخص النتائج ===\n";
    echo "تم تعيين المديرين بنجاح: {$successCount}\n";
    echo "فشل في التعيين: {$errorCount}\n";
