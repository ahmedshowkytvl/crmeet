...
# -*- coding: utf-8 -*-
"""
Test processing time calculation on specific ticket
"""
...
def test_specific_ticket():
    """Test processing time on specific ticket"""
    zoho = ZohoAPI()
    ...
    # Get ticket details for #2818683
    ticket_response = zoho.make_request('GET', f"{zoho.config.BASE_URLS['desk']}/tickets/2818683", 
    ...
    print(f"🎫 Testing Ticket #{ticket.get('ticketNumber')}")
    ...
    threads_response = zoho.make_request('GET', f"{zoho.config.BASE_URLS['desk']}/tickets/2818683/threads", 
                                       params={'orgId': zoho.config.ORG_ID})
    ...
    if threads_response and 'data' in threads_response:
        ...
        for i, thread in enumerate(threads, 1):
            print(f"   {i}. {thread.get('direction', 'Unknown')} - {thread.get('createdTime', 'No time')}")
