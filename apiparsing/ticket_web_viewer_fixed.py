...
# -*- coding: utf-8 -*-
"""
Web interface for viewing tickets - Fixed version with better error handling
"""
...
def get_tickets_stats():
    ...
    try:
        print("API: Getting ticket statistics...")
        zoho = ZohoAPI()
        ...
        print(f"API: Stats - Total: {total_tickets}, Auto Close: {len(auto_close_tickets)}, Excluding: {stats['excluding_auto_close']}")
        return jsonify({
            'success': True,
            'stats': stats
        })
        ...
        print(f"API: Error in get_tickets_stats: {e}")
        traceback.print_exc()
        return jsonify({'error': f'Server error: {str(e)}'}), 500

@app.route('/api/tickets')
def get_tickets():
    """Get last 20 tickets (excluding Auto Close)"""
    try:
        print("API: Getting tickets...")
        zoho = ZohoAPI()
        
        # Get exclude_auto_close parameter from request (default: True)
        exclude_auto_close = request.args.get('exclude_auto_close', 'true').lower() == 'true'
        ...
        for attempt in range(3):
            try:
                token = zoho.get_access_token()
                if token:
                    print(f"API: Access token obtained on attempt {attempt + 1}")
                    break
                else:
                    print(f"API: Access token attempt {attempt + 1} failed")
            except Exception as e:
                print(f"API: Access token attempt {attempt + 1} error: {e}")
            
            if attempt < 2:
                print("API: Waiting before retry...")
                import time
                time.sleep(1)
        
        if not token:
            print("API: Failed to get access token after 3 attempts")
            return jsonify({'error': 'Failed to get access token after 3 attempts'}), 500
        
        print("API: Getting tickets from Zoho...")
        # Get more tickets to account for filtering
        tickets_response = zoho.get_tickets(limit=100)
        if not tickets_response or 'data' not in tickets_response:
            print("API: No tickets data received")
            return jsonify({'error': 'Failed to get tickets'}), 500
        
        all_tickets = tickets_response['data']
        print(f"API: Received {len(all_tickets)} tickets")
        
        # Filter out Auto Close tickets if requested
        if exclude_auto_close:
            tickets = [
                ticket for ticket in all_tickets
                if ticket.get('cf', {}).get('cf_closed_by') != 'Auto Close'
            ]
            print(f"API: After filtering Auto Close: {len(tickets)} tickets (excluded {len(all_tickets) - len(tickets)})")
        else:
            tickets = all_tickets
            print(f"API: No Auto Close filtering applied")
        
        # Limit to 20 tickets for display
        tickets = tickets[:20]
        print(f"API: Limiting to {len(tickets)} tickets for display")
        ...
        for i, ticket in enumerate(tickets):
            try:
                ...
                print(f"API: Error formatting ticket {ticket.get('id', 'unknown')}: {e}")
                continue
        
        print(f"API: Returning {len(formatted_tickets)} formatted tickets")
        return jsonify({
            'success': True,
            'tickets': formatted_tickets,
            'count': len(formatted_tickets),
            'exclude_auto_close': exclude_auto_close,
            'total_before_filter': len(all_tickets) if exclude_auto_close else len(formatted_tickets)
        })
    
    except Exception as e:
        print(f"API: Error in get_tickets: {e}")
        traceback.print_exc()
        return jsonify({'error': f'Server error: {str(e)}'}), 500
...
def get_ticket_details(ticket_id):
    ...
    try:
        print(f"API: Getting details for ticket {ticket_id}")
        zoho = ZohoAPI()
        ...
        print(f"API: Getting ticket details from Zoho...")
        # Get ticket details
        ticket_response = zoho.make_request('GET', f"{zoho.config.BASE_URLS['desk']}/tickets/{ticket_id}", 
                                          params={'orgId': zoho.config.ORG_ID})
        
        if not ticket_response:
            print(f"API: Ticket {ticket_id} not found")
            return jsonify({'error': 'Ticket not found'}), 404
        
        print(f"API: Getting threads for ticket {ticket_id}")
        ...
        print(f"API: Formatting ticket details...")
        ...
        detailed_ticket = {
            ...
            'cf_closed_by': ticket_response.get('cf', {}).get('cf_closed_by', 'N/A'),
        ...
        print(f"API: Successfully formatted ticket details")
        return jsonify({
            'success': True,
        ...
        print(f"API: Error in get_ticket_details: {e}")
        traceback.print_exc()
        return jsonify({'error': f'Server error: {str(e)}'}), 500

if __name__ == '__main__':
    print("Starting Fixed Ticket Viewer...")
