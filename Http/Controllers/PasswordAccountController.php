...
class PasswordAccountController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $isAdmin = $user->email === 'admin@company.com' || 
                   ($user->role && in_array($user->role->slug, ['software_developer', 'admin']));
        
        if (!$isAdmin) {
            // Other users can only see assigned accounts
            $query->assignedTo($user->id);
        }
        // Admins and software developers can see all accounts (no filtering)
        ...
        if (!$isAdmin) {
            $allAccountsQuery->assignedTo($user->id);
        }
