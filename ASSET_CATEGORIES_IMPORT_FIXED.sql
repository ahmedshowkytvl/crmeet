-- فئات الأصول المستخرجة من تحليل البيانات (بدون عمود icon)
-- Asset Categories extracted from data analysis (without icon column)

-- 1. أجهزة الكمبيوتر
INSERT INTO asset_categories (name, name_ar, description, description_ar, is_active, created_at, updated_at) VALUES
('Desktop PC', 'أجهزة كمبيوتر مكتبية', 'Desktop computers and workstations', 'أجهزة كمبيوتر مكتبية ومحطات عمل', 1, NOW(), NOW()),
('Laptop', 'أجهزة لابتوب', 'Portable laptop computers', 'أجهزة لابتوب محمولة', 1, NOW(), NOW()),
('Server', 'خوادم', 'Server computers and workstations', 'خوادم وأجهزة خادم', 1, NOW(), NOW());

-- 2. أجهزة الشبكة
INSERT INTO asset_categories (name, name_ar, description, description_ar, is_active, created_at, updated_at) VALUES
('Modem', 'أجهزة مودم', 'Internet modems and connectivity devices', 'أجهزة مودم الإنترنت وأجهزة الاتصال', 1, NOW(), NOW()),
('Access Point', 'نقاط الوصول', 'Wireless access points', 'نقاط الوصول اللاسلكية', 1, NOW(), NOW()),
('Camera', 'كاميرات المراقبة', 'Security cameras and surveillance equipment', 'كاميرات الأمان وأجهزة المراقبة', 1, NOW(), NOW()),
('Camera-DVR', 'أجهزة DVR', 'Digital Video Recorders for surveillance', 'مسجلات الفيديو الرقمية للمراقبة', 1, NOW(), NOW());

-- 3. أجهزة الاتصالات
INSERT INTO asset_categories (name, name_ar, description, description_ar, is_active, created_at, updated_at) VALUES
('Avaya Phone', 'هواتف أفايا', 'Avaya telephone systems', 'أنظمة الهواتف أفايا', 1, NOW(), NOW()),
('Land Line', 'خطوط أرضية', 'Landline telephone connections', 'اتصالات الهاتف الأرضي', 1, NOW(), NOW());

-- 4. أجهزة الطاقة
INSERT INTO asset_categories (name, name_ar, description, description_ar, is_active, created_at, updated_at) VALUES
('UPS', 'مصادر طاقة احتياطية', 'Uninterruptible Power Supply units', 'وحدات إمداد الطاقة غير المنقطعة', 1, NOW(), NOW());

-- 5. الأثاث والمعدات
INSERT INTO asset_categories (name, name_ar, description, description_ar, is_active, created_at, updated_at) VALUES
('Furniture', 'أثاث', 'Office furniture and fixtures', 'أثاث مكتبي وتركيبات', 1, NOW(), NOW()),
('Office Equipment', 'معدات مكتبية', 'General office equipment and supplies', 'معدات مكتبية عامة ومستلزمات', 1, NOW(), NOW());
...
UPDATE asset_category_properties 
SET options = '["Join","Not Join"]' 
WHERE category_id = 1 AND name = 'Domain Status';

-- Condition options for Furniture
UPDATE asset_category_properties 
SET options = '["Excellent","Good","Fair","Poor","Damaged"]' 
WHERE category_id = 11 AND name = 'Condition';
